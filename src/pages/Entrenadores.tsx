...
export default function Entrenadores() {
    ...
    async function borrar(id: number) {
        const confirm = await ask("¿Eliminar entrenador?", { title: "Confirmar", kind: "warning", okLabel: "Sí", cancelLabel: "No" });
        if (confirm) {
            const db = await Database.load("sqlite:globalfutsal.db");
    ...
    const entrenadoresFiltrados = entrenadores.filter(e => {
        const texto = (e.nombre_deportivo + e.nombre + e.apellidos).toLowerCase();
        ...
        <div className="p-8 min-h-screen bg-gray-50 text-navy ml-0">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-navy flex items-center gap-3">
                        <UserCog className="text-orange" /> Entrenadores
                    </h1>
                    <p className="text-silver-dim mt-1">{entrenadores.length} técnicos registrados</p>
                </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-wrap items-center gap-4">
                <div className="flex-1 min-w-[200px] flex items-center gap-3 bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">
                    ...
                    <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar entrenador..." className="bg-transparent outline-none w-full text-navy placeholder-gray-400" />
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        {entrenadoresFiltrados.map((e) => {
                            ...
                            return (
                                <tr key={e.id} className="hover:bg-gray-50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        <div className="font-bold text-navy">{e.nombre_deportivo}</div>
                                        <div className="text-xs text-gray-400">{e.nombre} {e.apellidos}</div>
                                    </td>
                                    <td className="p-4 text-center">
                                        <div className="flex justify-center gap-1">
                                            {getBandera(e.nacionalidad_principal_id) && <img src={getBandera(e.nacionalidad_principal_id)!} className="w-6 h-4 border border-gray-200 shadow-sm" />}
                                    ...
                                    <td className="p-4 text-right flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => abrirFicha(e)} className="p-1.5 text-green-600 hover:bg-green-50 rounded bg-white border border-gray-200"><Eye size={16} /></button>
                                        <button onClick={() => abrirEditar(e)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded bg-white border border-gray-200"><Edit size={16} /></button>
                                        <button onClick={() => borrar(e.id)} className="p-1.5 text-red hover:bg-red-50 rounded bg-white border border-gray-200"><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            );
                        })}
                ...
                {entrenadoresFiltrados.length === 0 && <div className="p-10 text-center text-gray-400">No se encontraron entrenadores.</div>}
            </div>

            {/* --- MODAL FORMULARIO --- */}
            <Modal isOpen={modalFormOpen} onClose={() => setModalFormOpen(false)} title={editingId ? "Editar Entrenador" : "Nuevo Entrenador"}>
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div className="flex-1">
                            ...
                            <input value={formData.apodo} onChange={e => setFormData({ ...formData, apodo: e.target.value })} className="w-full p-2 border rounded font-bold text-navy" placeholder="Ej: Jesús Velasco" />
                        </div>
                    </div>
                    ...
                    <div className="bg-[#0B1F3B] w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-white/10">
                        <div className="relative p-8 flex items-center gap-6 border-b border-white/10 bg-gradient-to-r from-[#0B1F3B] to-[#16294a]">
                            ...
                            <div>
                                ...
                                <span className="bg-orange text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider mb-2 inline-block">Entrenador</span>
